...
static int find_free(int num_chunks) {
	...
	static unsigned int previous_block = 0; /* where find_free left off last time */

	int i,j;

	// search after where the previous run left off
	for(i=previous_block;i<max_chunk;i++) {
	...
	// search from 0 to the previous ending point
